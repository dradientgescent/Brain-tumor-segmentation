...
dense_model_path = 'trained_models/U_densenet/U_densenet.h5'
dense_weights_path = 'trained_models/U_densenet/SimUnet.01_0.047.hdf5'
dense_pb_path = 'trained_models/U_densene/densenet.pb'
...
st = time()
def get_grad_dice(test_image, gt):
    global layer_dice
    counter = 0
    for layer in range(1, len(model.layers)):
        if 'conv' in model.layers[layer].name:
            i = 1
            grads_ = visualize_cam(dummy_model, layer_idx, filter_indices=i, penultimate_layer_idx = layer,  
...
for model_type in ['dense']:
    ...
    layer_dice = np.zeros(num_layers)
    for i in range(num_images):
        test_image, gt = load_vol(test_path[i], model_type, slice_ = 78)
        get_grad_dice(test_image, gt)
        print (i, time() - st)

    # layer_dice = layer_dice[layer_dice != 0.]
